// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LeaverBusterLcdsMatchmakingNotification leaver buster lcds matchmaking notification
// swagger:model LeaverBusterLcdsMatchmakingNotification
type LeaverBusterLcdsMatchmakingNotification struct {

	// player join failures
	PlayerJoinFailures []interface{} `json:"playerJoinFailures"`
}

// Validate validates this leaver buster lcds matchmaking notification
func (m *LeaverBusterLcdsMatchmakingNotification) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LeaverBusterLcdsMatchmakingNotification) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LeaverBusterLcdsMatchmakingNotification) UnmarshalBinary(b []byte) error {
	var res LeaverBusterLcdsMatchmakingNotification
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
