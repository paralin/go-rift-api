// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LolCollectionsGameDataSplashMetadata lol collections game data splash metadata
// swagger:model LolCollectionsGameDataSplashMetadata
type LolCollectionsGameDataSplashMetadata struct {

	// calculated color
	CalculatedColor string `json:"CalculatedColor,omitempty"`

	// override color
	OverrideColor string `json:"OverrideColor,omitempty"`
}

// Validate validates this lol collections game data splash metadata
func (m *LolCollectionsGameDataSplashMetadata) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LolCollectionsGameDataSplashMetadata) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LolCollectionsGameDataSplashMetadata) UnmarshalBinary(b []byte) error {
	var res LolCollectionsGameDataSplashMetadata
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
