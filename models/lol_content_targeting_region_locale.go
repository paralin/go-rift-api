// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LolContentTargetingRegionLocale lol content targeting region locale
// swagger:model LolContentTargetingRegionLocale
type LolContentTargetingRegionLocale struct {

	// locale
	Locale string `json:"locale,omitempty"`
}

// Validate validates this lol content targeting region locale
func (m *LolContentTargetingRegionLocale) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LolContentTargetingRegionLocale) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LolContentTargetingRegionLocale) UnmarshalBinary(b []byte) error {
	var res LolContentTargetingRegionLocale
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
