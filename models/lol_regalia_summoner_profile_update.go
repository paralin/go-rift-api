// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LolRegaliaSummonerProfileUpdate lol regalia summoner profile update
// swagger:model LolRegaliaSummonerProfileUpdate
type LolRegaliaSummonerProfileUpdate struct {

	// key
	Key string `json:"key,omitempty"`

	// value
	Value string `json:"value,omitempty"`
}

// Validate validates this lol regalia summoner profile update
func (m *LolRegaliaSummonerProfileUpdate) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LolRegaliaSummonerProfileUpdate) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LolRegaliaSummonerProfileUpdate) UnmarshalBinary(b []byte) error {
	var res LolRegaliaSummonerProfileUpdate
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
