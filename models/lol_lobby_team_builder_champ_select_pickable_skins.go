// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LolLobbyTeamBuilderChampSelectPickableSkins lol lobby team builder champ select pickable skins
// swagger:model LolLobbyTeamBuilderChampSelectPickableSkins
type LolLobbyTeamBuilderChampSelectPickableSkins struct {

	// skin ids
	SkinIds []int32 `json:"skinIds"`
}

// Validate validates this lol lobby team builder champ select pickable skins
func (m *LolLobbyTeamBuilderChampSelectPickableSkins) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LolLobbyTeamBuilderChampSelectPickableSkins) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LolLobbyTeamBuilderChampSelectPickableSkins) UnmarshalBinary(b []byte) error {
	var res LolLobbyTeamBuilderChampSelectPickableSkins
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
