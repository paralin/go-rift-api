// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LolInventoryAccessTokenResource lol inventory access token resource
// swagger:model LolInventoryAccessTokenResource
type LolInventoryAccessTokenResource struct {

	// expiry
	Expiry int64 `json:"expiry,omitempty"`

	// scopes
	Scopes []string `json:"scopes"`

	// token
	Token string `json:"token,omitempty"`
}

// Validate validates this lol inventory access token resource
func (m *LolInventoryAccessTokenResource) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LolInventoryAccessTokenResource) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LolInventoryAccessTokenResource) UnmarshalBinary(b []byte) error {
	var res LolInventoryAccessTokenResource
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
