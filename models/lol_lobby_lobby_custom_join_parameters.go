// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LolLobbyLobbyCustomJoinParameters lol lobby lobby custom join parameters
// swagger:model LolLobbyLobbyCustomJoinParameters
type LolLobbyLobbyCustomJoinParameters struct {

	// as spectator
	AsSpectator bool `json:"asSpectator,omitempty"`

	// password
	Password string `json:"password,omitempty"`
}

// Validate validates this lol lobby lobby custom join parameters
func (m *LolLobbyLobbyCustomJoinParameters) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LolLobbyLobbyCustomJoinParameters) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LolLobbyLobbyCustomJoinParameters) UnmarshalBinary(b []byte) error {
	var res LolLobbyLobbyCustomJoinParameters
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
