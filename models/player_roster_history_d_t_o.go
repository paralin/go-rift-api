// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"strconv"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/swag"
)

// PlayerRosterHistoryDTO player roster history d t o
// swagger:model PlayerRosterHistoryDTO
type PlayerRosterHistoryDTO struct {

	// rosters
	Rosters []*RosterMemberDTO `json:"rosters"`
}

// Validate validates this player roster history d t o
func (m *PlayerRosterHistoryDTO) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateRosters(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *PlayerRosterHistoryDTO) validateRosters(formats strfmt.Registry) error {

	if swag.IsZero(m.Rosters) { // not required
		return nil
	}

	for i := 0; i < len(m.Rosters); i++ {
		if swag.IsZero(m.Rosters[i]) { // not required
			continue
		}

		if m.Rosters[i] != nil {
			if err := m.Rosters[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("rosters" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

// MarshalBinary interface implementation
func (m *PlayerRosterHistoryDTO) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *PlayerRosterHistoryDTO) UnmarshalBinary(b []byte) error {
	var res PlayerRosterHistoryDTO
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
