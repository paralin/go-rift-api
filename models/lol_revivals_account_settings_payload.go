// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LolRevivalsAccountSettingsPayload lol revivals account settings payload
// swagger:model LolRevivalsAccountSettingsPayload
type LolRevivalsAccountSettingsPayload struct {

	// data
	Data interface{} `json:"data,omitempty"`
}

// Validate validates this lol revivals account settings payload
func (m *LolRevivalsAccountSettingsPayload) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LolRevivalsAccountSettingsPayload) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LolRevivalsAccountSettingsPayload) UnmarshalBinary(b []byte) error {
	var res LolRevivalsAccountSettingsPayload
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
