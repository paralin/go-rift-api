// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LolInventoryRmsStoreEntitlementItem lol inventory rms store entitlement item
// swagger:model LolInventoryRmsStoreEntitlementItem
type LolInventoryRmsStoreEntitlementItem struct {

	// inventory type
	InventoryType string `json:"inventoryType,omitempty"`

	// item Id
	ItemID string `json:"itemId,omitempty"`
}

// Validate validates this lol inventory rms store entitlement item
func (m *LolInventoryRmsStoreEntitlementItem) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LolInventoryRmsStoreEntitlementItem) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LolInventoryRmsStoreEntitlementItem) UnmarshalBinary(b []byte) error {
	var res LolInventoryRmsStoreEntitlementItem
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
