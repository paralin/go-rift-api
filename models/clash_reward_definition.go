// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/swag"
)

// ClashRewardDefinition clash reward definition
// swagger:model ClashRewardDefinition
type ClashRewardDefinition struct {

	// reward spec
	RewardSpec *ClashRewardSpec `json:"rewardSpec,omitempty"`

	// reward type
	RewardType ClashRewardType `json:"rewardType,omitempty"`
}

// Validate validates this clash reward definition
func (m *ClashRewardDefinition) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateRewardSpec(formats); err != nil {
		res = append(res, err)
	}

	if err := m.validateRewardType(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *ClashRewardDefinition) validateRewardSpec(formats strfmt.Registry) error {

	if swag.IsZero(m.RewardSpec) { // not required
		return nil
	}

	if m.RewardSpec != nil {
		if err := m.RewardSpec.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("rewardSpec")
			}
			return err
		}
	}

	return nil
}

func (m *ClashRewardDefinition) validateRewardType(formats strfmt.Registry) error {

	if swag.IsZero(m.RewardType) { // not required
		return nil
	}

	if err := m.RewardType.Validate(formats); err != nil {
		if ve, ok := err.(*errors.Validation); ok {
			return ve.ValidateName("rewardType")
		}
		return err
	}

	return nil
}

// MarshalBinary interface implementation
func (m *ClashRewardDefinition) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *ClashRewardDefinition) UnmarshalBinary(b []byte) error {
	var res ClashRewardDefinition
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
