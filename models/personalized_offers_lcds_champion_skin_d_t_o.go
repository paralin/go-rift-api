// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// PersonalizedOffersLcdsChampionSkinDTO personalized offers lcds champion skin d t o
// swagger:model PersonalizedOffersLcdsChampionSkinDTO
type PersonalizedOffersLcdsChampionSkinDTO struct {

	// champion Id
	ChampionID int32 `json:"championId,omitempty"`

	// owned
	Owned bool `json:"owned,omitempty"`

	// skin Id
	SkinID int32 `json:"skinId,omitempty"`
}

// Validate validates this personalized offers lcds champion skin d t o
func (m *PersonalizedOffersLcdsChampionSkinDTO) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *PersonalizedOffersLcdsChampionSkinDTO) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *PersonalizedOffersLcdsChampionSkinDTO) UnmarshalBinary(b []byte) error {
	var res PersonalizedOffersLcdsChampionSkinDTO
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
