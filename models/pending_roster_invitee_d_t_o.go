// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/swag"
)

// PendingRosterInviteeDTO pending roster invitee d t o
// swagger:model PendingRosterInviteeDTO
type PendingRosterInviteeDTO struct {

	// invite time
	InviteTime int64 `json:"inviteTime,omitempty"`

	// invitee Id
	InviteeID int64 `json:"inviteeId,omitempty"`

	// invitee state
	InviteeState PendingRosterInviteeState `json:"inviteeState,omitempty"`

	// inviter
	Inviter int64 `json:"inviter,omitempty"`
}

// Validate validates this pending roster invitee d t o
func (m *PendingRosterInviteeDTO) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateInviteeState(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *PendingRosterInviteeDTO) validateInviteeState(formats strfmt.Registry) error {

	if swag.IsZero(m.InviteeState) { // not required
		return nil
	}

	if err := m.InviteeState.Validate(formats); err != nil {
		if ve, ok := err.(*errors.Validation); ok {
			return ve.ValidateName("inviteeState")
		}
		return err
	}

	return nil
}

// MarshalBinary interface implementation
func (m *PendingRosterInviteeDTO) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *PendingRosterInviteeDTO) UnmarshalBinary(b []byte) error {
	var res PendingRosterInviteeDTO
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
