// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LootSimpleDialogMessageResponse loot simple dialog message response
// swagger:model LootSimpleDialogMessageResponse
type LootSimpleDialogMessageResponse struct {

	// account Id
	AccountID int64 `json:"accountId,omitempty"`

	// command
	Command string `json:"command,omitempty"`

	// msg Id
	MsgID string `json:"msgId,omitempty"`
}

// Validate validates this loot simple dialog message response
func (m *LootSimpleDialogMessageResponse) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LootSimpleDialogMessageResponse) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LootSimpleDialogMessageResponse) UnmarshalBinary(b []byte) error {
	var res LootSimpleDialogMessageResponse
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
