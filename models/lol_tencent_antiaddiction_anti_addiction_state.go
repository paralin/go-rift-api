// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/swag"
)

// LolTencentAntiaddictionAntiAddictionState lol tencent antiaddiction anti addiction state
// swagger:model LolTencentAntiaddictionAntiAddictionState
type LolTencentAntiaddictionAntiAddictionState struct {

	// localization key
	LocalizationKey string `json:"localizationKey,omitempty"`

	// policy type
	PolicyType LolTencentAntiaddictionPolicyType `json:"policyType,omitempty"`
}

// Validate validates this lol tencent antiaddiction anti addiction state
func (m *LolTencentAntiaddictionAntiAddictionState) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validatePolicyType(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *LolTencentAntiaddictionAntiAddictionState) validatePolicyType(formats strfmt.Registry) error {

	if swag.IsZero(m.PolicyType) { // not required
		return nil
	}

	if err := m.PolicyType.Validate(formats); err != nil {
		if ve, ok := err.(*errors.Validation); ok {
			return ve.ValidateName("policyType")
		}
		return err
	}

	return nil
}

// MarshalBinary interface implementation
func (m *LolTencentAntiaddictionAntiAddictionState) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LolTencentAntiaddictionAntiAddictionState) UnmarshalBinary(b []byte) error {
	var res LolTencentAntiaddictionAntiAddictionState
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
