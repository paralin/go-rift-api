// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LolCatalogGameDataSummonerEmote lol catalog game data summoner emote
// swagger:model LolCatalogGameDataSummonerEmote
type LolCatalogGameDataSummonerEmote struct {

	// description
	Description string `json:"description,omitempty"`

	// id
	ID int64 `json:"id,omitempty"`

	// inventory icon
	InventoryIcon string `json:"inventoryIcon,omitempty"`

	// name
	Name string `json:"name,omitempty"`
}

// Validate validates this lol catalog game data summoner emote
func (m *LolCatalogGameDataSummonerEmote) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LolCatalogGameDataSummonerEmote) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LolCatalogGameDataSummonerEmote) UnmarshalBinary(b []byte) error {
	var res LolCatalogGameDataSummonerEmote
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
