// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// LolPerksGetGameCustomizationDTO lol perks get game customization d t o
// swagger:model LolPerksGetGameCustomizationDTO
type LolPerksGetGameCustomizationDTO struct {

	// queue type
	QueueType string `json:"queueType,omitempty"`
}

// Validate validates this lol perks get game customization d t o
func (m *LolPerksGetGameCustomizationDTO) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *LolPerksGetGameCustomizationDTO) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *LolPerksGetGameCustomizationDTO) UnmarshalBinary(b []byte) error {
	var res LolPerksGetGameCustomizationDTO
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
